...
static int tls_cryptoapi_cert(SSL *ssl, const char *name)
{
	...
	cert = d2i_X509(NULL, (unsigned char **) &priv->cert->pbCertEncoded,
		    ...
		    char **ppin, const char *key_id)
{
	...
	if (ENGINE_init(conn->engine) != 1) {
		...
		wpa_printf(MSG_ERROR, "ENGINE: engine init failed "
			   "(engine: %s) [%s]", engine_id, error);
		goto err;
	}
	wpa_printf(MSG_DEBUG, "ENGINE: engine initialized");

	if (ENGINE_ctrl_cmd_string(conn->engine, "PIN", *ppin, 0) == 0) {
		char *error = ERR_error_string(ERR_get_error(), NULL);
		wpa_printf(MSG_ERROR, "ENGINE: can't set pin [%s]", error);
		goto err;
	}
...
int tls_global_set_verify(void *ssl_ctx, int check_crl)
{
	...
	if (check_crl) {
		...
		if (cs == NULL) {
			wpa_printf(MSG_INFO, "TLS: Failed to get certificate "
				   "store when enabling check_crl: %s",
				   ERR_error_string(ERR_get_error(), NULL));
			return -1;
		}
				      ...
				      struct tls_connection *conn)
{
	...
	if (SSL_use_PrivateKey(conn->ssl, conn->private_key) != 1) {
		char *error = ERR_error_string(ERR_get_error(), NULL);
		wpa_printf(MSG_ERROR, "ENGINE: can't use private key for TLS "
			   "[%s]", error);
		return -1;
	}
	if (!SSL_check_private_key(conn->ssl)) {
		wpa_printf(MSG_INFO, "SSL: Private key failed verification: "
			       ...
			       const char *private_key_passwd)
{
	...
	if (SSL_use_PrivateKey_file(conn->ssl, private_key,
	    ...
	    tls_cryptoapi_cert(conn->ssl, private_key)) {
