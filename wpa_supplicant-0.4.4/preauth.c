				 ...
				 void *ctx)
{
	...
	if (success) {
		...
		if (res == 0) {
			...
			wpa_msg(sm->ctx, MSG_INFO, "RSN: failed to get master "
	...
	wpa_msg(sm->ctx, MSG_INFO, "RSN: pre-authentication with " MACSTR
...
static int rsn_preauth_eapol_send(void *ctx, int type, u8 *buf, size_t len)
{
	...
	res = l2_packet_send(sm->l2_preauth, msg, msglen);
	free(msg);
	return res;
}
...
int rsn_preauth_init(struct wpa_sm *sm, const u8 *dst, struct wpa_ssid *config)
{
	...
	wpa_msg(sm->ctx, MSG_DEBUG, "RSN: starting pre-authentication with "
		MACSTR, MAC2STR(dst));

	sm->l2_preauth = l2_packet_init(sm->ifname, sm->own_addr,
					ETH_P_RSN_PREAUTH,
					rsn_preauth_receive, sm);
	if (sm->l2_preauth == NULL) {
...
void rsn_preauth_candidate_process(struct wpa_sm *sm)
{
	...
	while (sm->pmksa_candidates) {
		...
		if (memcmp(sm->bssid, candidate->bssid, ETH_ALEN) != 0 &&
			...
			wpa_msg(sm->ctx, MSG_DEBUG, "RSN: PMKSA candidate "
		...
		wpa_msg(sm->ctx, MSG_DEBUG, "RSN: PMKSA candidate " MACSTR
	...
	wpa_msg(sm->ctx, MSG_DEBUG, "RSN: no more pending PMKSA candidates");
}
			 ...
			 int prio, int preauth)
{
	...
	wpa_msg(sm->ctx, MSG_DEBUG, "RSN: added PMKSA cache "
