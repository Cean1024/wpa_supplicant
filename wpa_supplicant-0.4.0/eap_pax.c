/*
 * WPA Supplicant / EAP-PAX (draft-clancy-eap-pax-03.txt)
...
static void * eap_pax_init(struct eap_sm *sm)
{
	...
	if (config == NULL || !config->nai || !config->eappsk) {
		wpa_printf(MSG_INFO, "EAP-PAX: CID (nai) or key (eappsk) not "
			   "configured");
		return NULL;
	}

	if (config->eappsk_len != EAP_PAX_AK_LEN) {
...
static struct eap_pax_hdr * eap_pax_alloc_resp(struct eap_pax_hdr *req,
...
static u8 * eap_pax_process_std_1(struct eap_sm *sm, struct eap_pax_data *data,
				  ...
				  size_t *respDataLen)
{
	struct eap_pax_hdr *req, *resp;
	u8 *pos;
	size_t left;

	wpa_printf(MSG_DEBUG, "EAP-PAX: PAX_STD-1 (received)");
	req = (struct eap_pax_hdr *) reqData;
	...
	pos = (u8 *) (req + 1);
	if (((pos[0] << 8) | pos[1]) != EAP_PAX_RAND_LEN) {
	...
	*pos++ = 0;
	*pos++ = EAP_PAX_RAND_LEN;
	memcpy(pos, data->rand.r.y, EAP_PAX_RAND_LEN);
	wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: B = Y (client rand)",
		    pos, EAP_PAX_RAND_LEN);
	pos += EAP_PAX_RAND_LEN;

	*pos++ = data->cid_len >> 8;
	*pos++ = data->cid_len & 0xff;
	memcpy(pos, data->cid, data->cid_len);
	wpa_hexdump_ascii(MSG_MSGDUMP, "EAP-PAX: CID", pos, data->cid_len);
	pos += data->cid_len;
				  ...
				  size_t *respDataLen)
{
	...
	pos = (u8 *) (req + 1);
	if (((pos[0] << 8) | pos[1]) != EAP_PAX_MAC_LEN) {
			    ...
			    size_t *respDataLen)
{
	...
	u8 *resp, *icv, icvbuf[EAP_PAX_ICV_LEN];
	size_t len;

	req = (struct eap_pax_hdr *) reqData;
	if (reqDataLen < sizeof(*req) || req->type != EAP_TYPE_PAX ||
	    (len = be_to_host16(req->length)) > reqDataLen ||
	    len < sizeof(*req) + EAP_PAX_ICV_LEN) {
	...
	wpa_hexdump(MSG_MSGDUMP, "EAP-PAX: received payload",
		    (u8 *) (req + 1), len - sizeof(*req) - EAP_PAX_ICV_LEN);
	...
	if (len < sizeof(*req) + EAP_PAX_ICV_LEN) {
		wpa_printf(MSG_INFO, "EAP-PAX: ICV underflow");
		ret->ignore = TRUE;
		return NULL;
	}
	len -= EAP_PAX_ICV_LEN;
	...
	if (req->op_code == EAP_PAX_OP_STD_1) {
		eap_pax_mac(req->mac_id, (u8 *) "", 0,
			    reqData, len, NULL, 0, NULL, 0,
			    icvbuf);
	} else {
		eap_pax_mac(req->mac_id, data->ick, EAP_PAX_ICK_LEN,
			    reqData, len, NULL, 0, NULL, 0, icvbuf);
	}
	...
	case EAP_PAX_OP_STD_1:
		resp = eap_pax_process_std_1(sm, data, ret, reqData, len,
					     respDataLen);
		break;
	case EAP_PAX_OP_STD_3:
		resp = eap_pax_process_std_3(sm, data, ret, reqData, len,
					     respDataLen);
		break;
	default:
