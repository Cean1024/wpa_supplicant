...
int wpa_eapol_send_preauth(void *ctx, int type, u8 *buf, size_t len)
{
	...
	msg = wpa_alloc_eapol(wpa_s, wpa_s->preauth_bssid, ETH_P_RSN_PREAUTH,
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_DISASSOC:
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "- Disconnect "
			"event - remove keys");
		if (wpa_supplicant_dynamic_keys(wpa_s)) {
			wpa_s->keys_cleared = 0;
...
int wpa_supplicant_reload_configuration(struct wpa_supplicant *wpa_s)
{
	...
	pmksa_cache_notify_reconfig(wpa_s);
	rsn_preauth_deinit(wpa_s);
	wpa_config_free(wpa_s->conf);
	wpa_s->conf = conf;
	if (reconf_ctrl)
				     ...
				     struct wpa_ssid *ssid)
{
	...
	if (bss && (bss->wpa_ie_len || bss->rsn_ie_len) &&
		...
		if (wpa_s->cur_pmksa == NULL && ssid->proactive_key_caching &&
		    (ssid->proto & WPA_PROTO_RSN)) {
			wpa_s->cur_pmksa = pmksa_cache_get_opportunistic(
				wpa_s, wpa_s->current_ssid, bss->bssid);
		}
		if (wpa_s->cur_pmksa) {
...
int main(int argc, char *argv[])
{
	...
	for (;;) {
		c = getopt(argc, argv, "Bc:D:dehi:KLNP:qtvwW");
		if (c < 0)
			break;
		switch (c) {
		case 'B':
