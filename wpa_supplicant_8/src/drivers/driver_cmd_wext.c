...
int wpa_driver_wext_driver_cmd( void *priv, char *cmd, char *buf, size_t buf_len )
{
	...
	if (os_strcasecmp(cmd, "RSSI-APPROX") == 0) {
		...
		os_sleep(0, 50000);	/* hhuang, 20120130, fix START/STOP */
	...
	if (ret < 0) {
		...
		if ((os_strcasecmp(cmd, RSSI_CMD) == 0) ||
			...
			os_sleep(0, 500000); /* wyzhao, 20120522, fix START : when STOP, then START might cause START not work well*/
			linux_set_iface_flags(drv->ioctl_sock, drv->ifname, 1);
			os_sleep(1, 0);		/* hhuang, 20120130, fix START/STOP */
