...
static int wpa_driver_ipw_set_param(void *priv, const char *param)
{
	struct wpa_driver_ipw_data *drv = priv;
	int ret = 0;
	u8 name;
	...
	name = param[0];
	value = param[1]|(param[2] << 8)|(param[3] << 16)|(param[4]<<24);

	if (ipw_set_wpa_param(drv, name,
			      value) < 0) {
		printf("ipw: Fail notify encryption type to upper layer!\n");
		ret = -1;
	}
...
const struct wpa_driver_ops wpa_driver_ipw_ops = {
	...
	.set_param = wpa_driver_ipw_set_param,
};
