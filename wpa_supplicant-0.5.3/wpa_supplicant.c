...
//delete by david intentionly for it used to deal nothing, 2006.9.26
#if 0	
...
{
	struct wpa_supplicant *wpa_s;
	u8 wpax_select[5] = {0,0,0,0,0};//1.name; 4:value
	...
	/**
	 * The WPA setting shall including following 3 part.
	 * [1] Encryption type: wpa/wpa2,
	 * [2] Pairwise cipher type,
	 * [3] Authen type: open or 802.1x
	 * ..
	 */
	wpax_select[0] = IPW_PARAM_WPAX_SELECT;

	if (wpa_s->conf->ssid->proto&WPA_PROTO_WPA) {
	  wpax_select[1] = IPW_PROTO_WPA;
	} else if(wpa_s->conf->ssid->proto&WPA_PROTO_RSN) {
	  wpax_select[1] = IPW_PROTO_RSN;
	}
	// Cipher Suite Selection
	if(wpa_s->conf->ssid->pairwise_cipher&WPA_CIPHER_NONE) {
		wpax_select[2] = IPW_WPAX_USEGROUP;
	} else if(wpa_s->conf->ssid->pairwise_cipher&WPA_CIPHER_WEP40) {
		wpax_select[2] = IPW_WPAX_WEP40;
	} else if(wpa_s->conf->ssid->pairwise_cipher&WPA_CIPHER_TKIP) {
		wpax_select[2] = IPW_WPAX_TKIP;
	} else if(wpa_s->conf->ssid->pairwise_cipher&WPA_CIPHER_CCMP) {
		wpax_select[2] = IPW_WPAX_CCMP;
	} else if(wpa_s->conf->ssid->pairwise_cipher&WPA_CIPHER_WEP104) {
		wpax_select[2] = IPW_WPAX_WEP104;
	} //else CIPHER NONE

	//Authentication and Key Management Suite Selectors
	if((wpa_s->conf->ssid->key_mgmt&WPA_KEY_MGMT_PSK) || 
			(wpa_s->conf->ssid->key_mgmt&WPA_KEY_MGMT_NONE)) {
		wpax_select[3] = IPW_KEY_MGMT_PSK;
	} else if(wpa_s->conf->ssid->key_mgmt&WPA_KEY_MGMT_IEEE8021X) {
		wpax_select[3] = IPW_KEY_MGMT_IEEE8021X;
	} else {
		wpax_select[3] = IPW_KEY_MGMT_PSK;
	} //else CIPHER NONE
           ...
           int i;
	   for (i = 0; i < 5; i++) {
	   	printf("%02x-",wpax_select[i]);
	   }
	   printf("\n");
	}
#endif	
	if (wpa_drv_set_param(wpa_s, wpax_select) < 0) {
		wpa_printf(MSG_ERROR, "Driver interface rejected "
			   "driver_param '%s'", wpax_select);
		//printf("drv set data error!\n");
		return NULL;
	} else {
		//printf("drv set param ok!>>>>>>>>>>>>>>>>>>>\n");
